  ...
  while IFS='=' read -r var value; do
    ...
    if [[ $var != '#'* ]] && [[ $var != '//'* ]] && [[ $var ]]; then
      ...
      if ! printenv | grep -q "^$var="; then
        ...
        echo "Skipping $var as it already exists"
        env | grep "$var"
      fi
    fi
  done <"$TS_CONF_PATH"/ts.ini
...
if [ ! -s $TS_CONF_PATH/qBittorrent/config/qBittorrent.conf ]; then
  mkdir -p $TS_CONF_PATH/qBittorrent/config && chmod -R 777 $TS_CONF_PATH/qBittorrent/config
...
[ -s "$TS_STAT" ] && [ $(
  jq empty $TS_STAT >/dev/null 2>&1
  echo $?
) -eq 0 ] && jq '."monitor" = {}' $TS_STAT | sponge $TS_STAT
...
if [ ! -z "$UPDATE_TASK" ]; then
  crontab -l | {
    cat
    echo "$(echo "$UPDATE_TASK" | sed 's/\\//g' | sed "s/\"//g") /update.sh >> /var/log/cron.log 2>&1"
  } | crontab -
fi
...
if [ "$QBT_ENABLED" == "true" ]; then
  ...
  export POST_AUTH=""
  if [ $(
    jq empty $TS_CONF_PATH/accs.db >/dev/null 2>&1
    echo $?
  ) -eq 0 ] && [ "$(echo "$TS_OPTIONS" | grep "\-a")" ]; then
  ...
  crontab -l | {
    cat
    echo "$(echo "$QBT_CHECKS_TASK" | sed 's/\\//g' | sed "s/\"//g") /qbt_manager.sh >> /var/log/cron.log 2>&1"
  } | crontab -
  ...
  if [ ! -z "$QBT_RESUME_HOUR" ]; then
    ...
    crontab -l | {
      cat
      echo "$(echo "$QBT_RESUME_TASK" | sed 's/\\//g' | sed "s/\"//g") /qbt_resume_torrents.sh >> /var/log/cron.log 2>&1"
    } | crontab -
  fi
